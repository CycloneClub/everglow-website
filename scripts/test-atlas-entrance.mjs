...
test('a burst of downward wheel events cannot interrupt or restart the entrance', () => {
  ...
  for (let time = 50; time < 1500; time += 50) {
    scene.tick(time)
...
test('the paper lifts gently, travels through the middle, and settles before reading begins', () => {
  const scene = setup()
  scene.snap.advance(0, 80)
  scene.tick(100)
  assert.ok(scene.position() > 0 && scene.position() < 10, 'the first movement should be a gentle lift')
  scene.tick(750)
  assert.ok(scene.position() > 300 && scene.position() < 500, 'the paper is still crossing the hero')
  scene.tick(1400)
  assert.ok(scene.position() > 790 && scene.position() < 800, 'the final movement should settle softly')
  scene.tick(1500)
  assert.equal(scene.position(), 800)
})
