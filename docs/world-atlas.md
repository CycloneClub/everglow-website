...
首页顺序为开场 → 世界树手记 → 旅程入口 → 新闻。首屏向下滚动一次，会用约 700ms 吸附到手记开头：首屏保持原位，完整不透明的羊皮纸从下方覆盖。连续同向滚动不取消吸附，也不重置动画时间；不可取消的滚轮事件造成的位置偏移会在动画及短暂收尾期内校正。动画途中或刚到手记顶部时，向上滚会反向吸附回首屏；退出途中向下滚则重新进入。反向动画从当前位置开始，耗时按剩余距离缩短。支持滚轮、触屏及双向翻页键，收尾期最多 360ms，进入章节后恢复原生上下阅读。点击、调整窗口、离开页面会取消动画及收尾校正；减少动态效果模式保留原生滚动。仅此入口自动吸附，世界树章节内部仍只读取原生滚动位置，不按时间自行播放。
...
手机动态阅读的页脚提供“上一则 / 下一则”：依次停在完整树、每层标题、四条批注、未知界层、完整树结语；结语之后按钮变为“继续旅程”。停靠点由界层时间线生成，位于内容完整可见的区间。按钮翻阅由 `atlas-scroll.ts` 控制：同一界层内部的内容切换为 1500ms，进入界层、跨层拉远升高、未知区域与返回完整树等大幅镜头移动为 3000ms，正反方向规则一致。采用五次 smootherstep 缓入缓出曲线，起止速度和加速度均为零；继续经过原有镜头与淡入淡出。连续点按按目标位置递进，改向从当前实际位置重新过渡。触屏滑动、滚轮、其他键盘操作、模式切换或离开页面会取消按钮翻阅，恢复自然滚动，不添加逐项吸附。桌面页脚不变。
...
“略过手记”和结尾的“继续旅程”共用快速退出：用 700ms 缓入缓出滚动，将旅程入口底部（新闻上边界）对齐当前可见视口下沿。目标随可见视口变化实时重算，完成后仅转移焦点，不再次滚动；手动输入仍可打断。减少动态效果偏好下直接定位到相同边界。
