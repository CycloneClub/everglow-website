...
首页顺序为开场 → 世界树手记 → 旅程入口 → 新闻。首屏向下滚动一次，会用约 700ms 吸附到手记开头：首屏保持原位，完整不透明的羊皮纸从下方覆盖。连续同向滚动不取消吸附，也不重置动画时间；不可取消的滚轮事件造成的位置偏移会在动画及短暂收尾期内校正。动画途中或刚到手记顶部时，向上滚会反向吸附回首屏；退出途中向下滚则重新进入。反向动画从当前位置开始，耗时按剩余距离缩短。支持滚轮、触屏及双向翻页键，收尾期最多 360ms，随后把桌面滚轮交给章节停靠点控制。点击、调整窗口、离开页面会取消动画及收尾校正；减少动态效果模式保留原生滚动。入口与章节吸附按顺序接管输入，不同时驱动滚动；触屏滑动仍保留原生阅读。

## 时间线与地图

首屏底部中央的向上双箭头可点击进入手记，复用同一入口吸附控制器；支持键盘和三语可访问名称。箭头轻微上下浮动，减少动态效果偏好下停止浮动并直接定位。无 JavaScript 时保留 `#lore` 锚点跳转。
...
完整树 → 树纹与地图结构浮现并停留在“Between these lines”题注 → 天穹镇 → 拉远并向上 → 苍苔帘幕 → 上方未知节点 → 完整树 → “Hang on to your dreams.” → 纸面退出。所有变化可逆，直接拖动滚动条也会落到对应状态。
...
手机动态阅读的页脚提供“上一则 / 下一则”：依次停在完整树、地图题注、每层标题、四条批注、未知界层、完整树结语；结语之后按钮变为“继续旅程”。停靠点由界层时间线生成，位于内容完整可见的区间。按钮与 PC 滚轮翻阅共用 `atlas-scroll.ts` 中的翻页控制器：同一界层内部的内容切换为 1500ms，进入界层、跨层拉远升高、未知区域与返回完整树等大幅镜头移动为 3000ms，正反方向规则一致。采用五次 smootherstep 缓入缓出曲线，起止速度和加速度均为零；继续经过原有镜头与淡入淡出。翻页途中，同向点击或滚轮输入只保留当前目标，不排队，也不重启动画；反向输入交换本次翻页的出发点和目标，从当前位置平滑返回，耗时按剩余距离缩短（最低 180ms）。到位后按钮的新点击才能前进到下一则。PC 竖向滚轮复用同一组停靠点；到位后保留 180ms 的收尾，连续同向尾波将静默等待延长至最近一次输入后 160ms，并校正不能阻止原生滚动的事件造成的偏移。反向滚轮可立即开始上一则。触屏滑动、非翻页键盘操作、拖动滚动条、窗口尺寸变化、模式切换或离开页面会取消翻页。桌面页脚和原有内容布局保持不变。
...
“略过手记”、结尾的“继续旅程”与 PC 最后一个停靠点继续下滚均采用快速退出：用 700ms 缓入缓出滚动，将旅程入口底部（新闻上边界）对齐当前可见视口下沿。目标随可见视口变化实时重算，完成后仅转移焦点，不再次滚动；手动输入仍可打断。减少动态效果偏好下直接定位到相同边界。
