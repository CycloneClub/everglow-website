<script setup lang="ts">
	...
	function compile(options?: { autoSave?: boolean }) {
		...
		if (fragment.ok === false)
			return fail(fragment.error, 'HLSL transpile failed');
	...
	<section class="hlsl-preview">
		<header class="hlsl-preview__header">
			...
			<div class="hlsl-preview__actions">
				...
				<span :class="['hlsl-preview__status', { 'is-error': error }]"
					>{{ status
					}}<template v-if="lastSavedAt">
						· saved {{ formatSavedAt(lastSavedAt) }}</template
					></span
				>
			</div>
		</header>
		...
		<div class="hlsl-preview__preview-row">
			...
			<section class="hlsl-textures">
				<div class="hlsl-textures__header">
					...
					><label class="hlsl-preview__button"
						><Icon name="lucide:plus" /> Import<input
							type="file"
							accept="image/*"
							@change="addTexture"
					/></label>
				</div>
				<p class="hlsl-textures__hint">Use <code>Textures.Name</code> in C#.</p>
				<p
		...
		<div class="hlsl-preview__board">
			...
			<div class="hlsl-preview__params">
				...
				>
					<h2>{{ panel.title }}</h2>
					<p v-if="!panel.values.length">No <code>@param</code> annotations.</p>
					<label
						v-for="param in panel.values"
						:key="param.id"
					>
						<span
							>{{ param.label }} <output>{{ param.value }}</output></span
						>
						<input
